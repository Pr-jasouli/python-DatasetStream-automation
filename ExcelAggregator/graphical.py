...
class SimpleGUI:
    ...
    def process_contracts(self):
        ...
        for file in xlsx_files:
            ...
            success = self.run_script('stage1.py', file_path)
            if not success:
        ...
        success_centralize = self.run_script('stage2.py', selected_directory)
        self.update_status_label("")
    ...
    def extract_results(self):
        ...
        internal_results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results')
        if not os.path.exists(internal_results_dir):
            messagebox.showerror("Error", "No results found to extract.", parent=self.root)
            return

        try:
            shutil.copytree(internal_results_dir, os.path.join(output_directory, 'results'))
            messagebox.showinfo("Success", f"Results have been saved to {output_directory}", parent=self.root)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to extract results: {e}", parent=self.root)
